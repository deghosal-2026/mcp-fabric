...
# --- obtain admin token for authenticated test scripts ---
info "Obtaining admin token..."
FABRIC_API_TOKEN=""
for _ in 1 2 3; do
    FABRIC_API_TOKEN=$(curl -sf -X POST "$API_URL/v1/auth/setup" \
        -H "Content-Type: application/json" \
        -d '{"username":"admin","password":"Admin123!","email":"admin@fabric.dev"}' 2>/dev/null \
        | python3 -c "import sys,json; print(json.load(sys.stdin).get('token',''))" 2>/dev/null)
    if [ -n "$FABRIC_API_TOKEN" ]; then
        pass "Admin token obtained"
        break
    fi
    sleep 2
done
if [ -z "$FABRIC_API_TOKEN" ]; then
    fail "Could not obtain admin token — tests requiring auth will fail"
fi
export FABRIC_API_TOKEN
...
run_test() {
    ...
    if bash "$script" "$API_URL" "$FABRIC_API_TOKEN"; then
        pass "$name"
        PASSED=$((PASSED + 1))
    else
        fail "$name"
