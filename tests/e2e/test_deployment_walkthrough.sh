...
resp=$(curl -sf -X POST "$API_URL/v1/agent-classes" \
    -H "Content-Type: application/json" \
    -d '{
...
resp=$(curl -sf -X POST "$API_URL/v1/agent-classes/$class_id/trust" \
    -H "Content-Type: application/json" \
    -d "{
        \"server_id\": \"$server_id\",
        \"trust_level\": \"trusted\"
...
resp=$(curl -sf -X POST "$API_URL/v1/agent-classes/$class_id/identities" \
    -H "Content-Type: application/json" \
    -d "{
        \"name\": \"dev-agent-01\",
...
resp=$(curl -sf "$API_URL/v1/audit?actor_id=$agent_id" 2>&1) || {
