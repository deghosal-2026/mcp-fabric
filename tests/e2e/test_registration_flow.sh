...
echo "--- GET /v1/servers ---"
resp=$(REQ "$API_URL/v1/servers" 2>&1) || fail "list servers request failed"
...
resp=$(REQ -X POST "$API_URL/v1/servers/$server_id/inspect" 2>&1) || fail "inspect request failed"
