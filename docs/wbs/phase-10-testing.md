...
### T10-01 to T10-15
Registry (register+inspect+list+decommission), Capability (create+map+resolve+conflict+deprecate+alias), Policy (evaluate+deploy+agent_class+trust), Routing (execute+fallback+batch), Audit (log+query+export), Approval (create+approve+deny+expire), Pack (create+assign+clone), Alert (fire+acknowledge), Auth (token lifecycle+admin login+MFA+password), Tenant (namespace filtering), Schema Diff (change detection), Errors (all 14 types), Pagination (cursor+offset), Batch (mixed results), Fallback (timeout+degrade+alert).
...
### T10-23: Migration Flow (empty for v0.1.0)
...
### T10-25: Policy Deploy Integration Test
...
### T10-26: Secure Fail-Closed Test
...
### T10-27: Docker Compose Smoke Test
...
### T10-29: First-Time Deployment Walkthrough
...
### T10-30: Mock MCP Server Fixture
