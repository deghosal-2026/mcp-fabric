...
AGENT_ID_MISMATCH_MSG = "agent_id in URL does not match authenticated agent"
    ...
    if agent_id is None:
        raise HTTPException(
            ...
            detail={
                ...
                "message": "Authentication required to manage webhooks",
            },
        )
    return str(agent_id)


def _assert_owns_agent_id(request: Request, agent_id: str) -> None:
    if _get_agent_id(request) != agent_id:
        raise HTTPException(
            status_code=403,
            detail={
                "error": "forbidden",
                "message": AGENT_ID_MISMATCH_MSG,
            },
        )
...
async def register_webhook(
    ...
    _assert_owns_agent_id(request, agent_id)
