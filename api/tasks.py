...
class NotifyTask(Task):  # type: ignore[misc]
...
def health_check_server(self: Any, server_id: str, endpoint: str) -> dict[str, Any]:
...
def health_check_all_servers(self: Any) -> list[dict[str, Any]]:
...
@celery_app.task(bind=True)  # type: ignore[untyped-decorator]
def cleanup_audit_logs(self: Any) -> dict[str, Any]:
    ...
    return _run_async(_cleanup())  # type: ignore[no-any-return]


@celery_app.task(bind=True)  # type: ignore[untyped-decorator]
def cleanup_expired_tokens(self: Any) -> dict[str, Any]:
...
@celery_app.task(bind=True)  # type: ignore[untyped-decorator]
def cleanup_expired_approvals(self: Any) -> dict[str, Any]:
...
@celery_app.task(bind=True)  # type: ignore[untyped-decorator]
def cleanup_expired_sessions(self: Any) -> dict[str, int]:
...
def check_alert_thresholds(self: Any) -> dict[str, int]:
...
@celery_app.task(base=NotifyTask, bind=True)  # type: ignore[untyped-decorator]
def notify_schema_change(
    ...
    tools_changed: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
...
def notify_approval_request(
    self: Any, approval_id: str, channels: list[str] | None = None
) -> dict[str, Any]:
...
@celery_app.task(bind=True, max_retries=3, default_retry_delay=60)  # type: ignore[untyped-decorator]
def deliver_alert(
    self: Any, alert_id: str, message: str, channels: list[str] | None = None
) -> dict[str, Any]:
...
def generate_audit_export(self: Any, event_types: list[str] | None = None) -> dict[str, Any]:
...
@celery_app.task(bind=True)  # type: ignore[untyped-decorator]
def run_scheduled_exports(self: Any) -> dict[str, int]:
...
@celery_app.task(bind=True)  # type: ignore[untyped-decorator]
def health_check_self(self: Any) -> dict[str, str]:
